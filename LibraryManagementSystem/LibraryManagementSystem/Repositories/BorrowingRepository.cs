...
namespace LibraryManagementSystem.Repositories
{
    public class BorrowingRepository(ApplicationDbContext dbContext) :Repository<Borrowing>(dbContext),IBorrowingRepository
    {
        ...
        public async Task<int> GetCountOfActiveBorrowingBookForThisMemberAsync(int memberId) =>
            await _set.Where(x => x.MemberId == memberId && x.ReturnDate == null).CountAsync();
        ...
        {
            Borrowing? borrowing = await _set.Where(x => x.BookId == bookId).OrderBy(x => x.BookId).LastOrDefaultAsync();
